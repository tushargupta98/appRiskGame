...
/**
 * This class checks whether the player is aggressive type or not
 * 
 * @author chowdhuryfarsadaurangzeb
 *
 */
public class AggressivePlayer implements PlayerStrategy, GameInterface {
  ...
  private String name = "Aggressive Player";
  ...
  /**
   * Actual method for Assigning armies to an aggressive player
   * 
   * @param noOfArmy
   */
  public AggressivePlayer(int noOfArmy) {
  ...
  public void attack() {
    ...
    // getting the list of all the conquered countries
    Country country = getListOfPlayersConqueredCountries().get(0);
    for (int i = 1; i < getListOfPlayersConqueredCountries().size(); i++) {
      if (country.getSoilders() < getListOfPlayersConqueredCountries().get(i).getSoilders()) {
        country = getListOfPlayersConqueredCountries().get(i);
      }
    }
    // checking for a country if attacker or not
    while (isCountryAttacker(country)) {
      Country attackableCountry = nextAttackableCountry(country);
      System.out
          .println(country.getCountryName() + " will attack " + attackableCountry.getCountryName());
      int noOfDiceOfAttacker = 0;
      // checking for the soldier count
      ...
      if (attackerDiceRoll > defenderDiceRoll) {
        ...
        // checking whether the soldier count of attackable country
  ...
  public void fortify() {
    ...
    for (int i = 0; i < getListOfPlayersConqueredCountries().size(); i++) {
      if (isCountryAttacker(getListOfPlayersConqueredCountries().get(i))) {
        if (country == null)
          country = getListOfPlayersConqueredCountries().get(i);
        else if (country.getSoilders() < getListOfPlayersConqueredCountries().get(i)
            .getSoilders()) {
          country = getListOfPlayersConqueredCountries().get(i);
        }
      }
    ...
    if (country != null) {
      ...
      if (neighbooringCountries != null && neighbooringCountries.size() > 0) {
        ...
        // Find the neighbooring country with largest soldier
        Country neighbooringCountry = neighbooringCountries.get(0);
        for (int i = 1; i < neighbooringCountries.size(); i++) {
          if (neighbooringCountries.get(i).getSoilders() > neighbooringCountry.getSoilders()) {
            neighbooringCountry = neighbooringCountries.get(i);
          }
        }
        // Fortify all the soldier to the main country
        System.out.println(
            country.getCountryName() + " moves in " + (neighbooringCountry.getSoilders() - 1)
                + " soldier from " + neighbooringCountry.getCountryName());
        country.moveInSoldier(neighbooringCountry.getSoilders() - 1);
        neighbooringCountry.moveOutSoldier(neighbooringCountry.getSoilders() - 1);
  ...
  public void reinforcement() {
      ...
      if (calculateMatchingRiskCard()) {
        ...
        for (int i = 0; i < RISK_CARD_NAMES.length && isContinue; i++) {
          ...
          for (int j = 0; j < getRiskCards().size(); j++) {
            // checking if the cardname equals risk card name
    ...
    if (getListOfPlayersConqueredCountries() != null
      ...
      // Looks for Country with largest number of Soldier
      Country countryWithLargestSoldier = getListOfPlayersConqueredCountries().get(0);
      for (int i = 1; i < getListOfPlayersConqueredCountries().size(); i++) {
        if (countryWithLargestSoldier.getSoilders() < getListOfPlayersConqueredCountries().get(i)
            .getSoilders()) {
          countryWithLargestSoldier = getListOfPlayersConqueredCountries().get(i);
        }
      }
      // Add all the unassigned soldier to that country.
      System.out.println(getName() + " reinforces " + getNoOfUnitUnassigned() + " army.");
      countryWithLargestSoldier.moveInSoldier(getNoOfUnitUnassigned());
  ...
  public int getControlValuePoint() {
    ...
    for (int i = 0; i < Map.getContinentList().size(); i++) {
      ...
      for (int j = 0; j < Map.getContinentList().get(i).getListOfCountriesInContinent()
          .size(); j++) {
        // checking condition for countries in a continent
  ...
  public ArrayList<Country> listOfNeighbooringSamePlayerCountries(Country country) {
    ...
    for (int i = 0; i < country.getNeighbooringCountries().size(); i++) {
      // checking condition for player strategy
  ...
  public boolean isCountryAttacker(Country country) {
    if (country.getSoilders() < 2)
      return false;
    // checking condition for neibhouring countries
    for (int i = 0; i < country.getNeighbooringCountries().size(); i++) {
